...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="scroll-smooth">
    ...
    <body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-200 min-h-screen flex flex-col antialiased">
        ...
        <!-- Header / Navbar -->
        <header x-data="navController()" class="sticky top-0 z-40 backdrop-blur-md bg-white/80 dark:bg-slate-900/80 border-b border-emerald-100 dark:border-emerald-900/30 transition-colors duration-300">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                        ...
                        <template x-if="isLoggedIn">
                            <div class="relative" x-data="{ open: false }" @click.away="open = false">
                                <button @click="open = !open" class="flex items-center gap-2 bg-slate-100 dark:bg-slate-800/80 hover:bg-slate-200 dark:hover:bg-slate-800 px-4 py-2 rounded-xl transition-all duration-200">
                                    ...
                                    <svg class="w-4 h-4 text-slate-600 font-medium dark:text-slate-300 transition-transform" :class="{'rotate-180': open}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                </button>
                                
                                <div x-show="open" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute right-0 mt-2 w-56 rounded-2xl bg-white dark:bg-slate-900 shadow-xl border border-emerald-100 dark:border-emerald-900/30 py-2 z-50">
                                    <div class="px-4 py-2 border-b border-emerald-100 dark:border-emerald-900/30">
                                        <p class="text-xs text-slate-600 font-medium dark:text-slate-300  font-medium dark:text-slate-400">Masuk sebagai</p>
                                    ...
                                    <hr class="border-emerald-100 dark:border-emerald-900/30 my-1">
                    ...
                    <!-- Mobile Menu Button -->
                    <div class="md:hidden">
                        <button @click="openMenu = !openMenu" class="p-2 text-slate-600 font-medium dark:text-slate-300 hover:text-slate-800 dark:hover:text-white">
            ...
            <!-- Mobile Menu -->
            <div x-show="openMenu" @click.away="openMenu = false" x-transition class="md:hidden bg-white dark:bg-slate-900 border-b border-emerald-100 dark:border-emerald-900/30 px-4 pt-2 pb-4 space-y-2">
                ...
                <hr class="border-emerald-100 dark:border-emerald-900/30 my-2">
                
                <template x-if="!isLoggedIn">
                    <div class="grid grid-cols-2 gap-2 pt-2">
                        <a href="{{ route('login') }}" @click="openMenu = false" class="text-center font-semibold text-slate-700 dark:text-slate-300 border border-emerald-100 dark:border-slate-700 py-2.5 rounded-xl">Masuk</a>
                ...
                <template x-if="isLoggedIn">
                    <div class="space-y-1">
                        ...
                        <a :href="isAdmin ? '{{ route('admin.profile') }}' : '{{ route('profile.settings') }}'" @click="openMenu = false" class="block px-3 py-2 text-base font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg">Pengaturan Profil</a>
                        <template x-if="!isAdmin">
                            <a href="{{ route('profile.history') }}" @click="openMenu = false" class="block px-3 py-2 text-base font-medium text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 rounded-lg">Riwayat Pembelian</a>
                        </template>
        ...
        <!-- Footer -->
        <footer class="bg-slate-900 text-slate-600 font-medium dark:text-slate-300  font-medium dark:text-slate-400 border-t border-slate-800 pt-16 pb-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-4">
                            <!-- Instagram -->
                            <a href="{{ \App\Models\SiteSetting::getValue('contact_instagram', 'https://instagram.com/sawahpulohub') }}" target="_blank" class="p-2 bg-slate-800 text-slate-600 font-medium dark:text-slate-300  font-medium dark:text-slate-400 hover:bg-emerald-600 hover:text-white rounded-xl transition-all">
                            ...
                            <!-- TikTok -->
                            <a href="{{ \App\Models\SiteSetting::getValue('contact_tiktok', 'https://tiktok.com/@sawahpulohub') }}" target="_blank" class="p-2 bg-slate-800 text-slate-600 font-medium dark:text-slate-300  font-medium dark:text-slate-400 hover:bg-emerald-600 hover:text-white rounded-xl transition-all">
                            ...
                            <!-- Facebook -->
                            <a href="https://facebook.com/sawahpulohub" target="_blank" class="p-2 bg-slate-800 text-slate-600 font-medium dark:text-slate-300  font-medium dark:text-slate-400 hover:bg-emerald-600 hover:text-white rounded-xl transition-all">
