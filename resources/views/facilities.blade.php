<x-tourist-layout>
    ...
    <section class="py-16 bg-white dark:bg-slate-900 transition-colors">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-slate-50 dark:bg-slate-800/40 shadow-sm border border-slate-100 dark:border-slate-800/60 rounded-3xl p-6 sm:p-10">
                <div class="grid grid-cols-1 lg:grid-cols-5 gap-10 items-center">
                    ...
                    <!-- Image Plan (3 cols) -->
                    <div class="lg:col-span-3 bg-white dark:bg-slate-950 p-2.5 rounded-2xl border border-slate-100 dark:border-slate-800 cursor-zoom-in" x-data="{ showModal: false }">
                        ...
                        <!-- Zoom Modal -->
                        <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-sm" x-cloak>
                            <div class="relative max-w-5xl w-full" @click.away="showModal = false">
                                <button @click="showModal = false" class="absolute -top-12 right-0 text-white hover:text-slate-300 text-sm font-semibold flex items-center gap-1">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    Tutup Denah
                                </button>
                                <img src="{{ \App\Models\SiteSetting::getValue('site_plan_image', 'https://images.unsplash.com/photo-1524661135-423995f22d0b?auto=format&fit=crop&w=1200&q=80') }}" alt="2D Site Plan Full" class="w-full h-auto rounded-2xl shadow-2xl max-h-[80vh] object-contain mx-auto" />
                            </div>
                        </div>
                    </div>
    ...
    <section class="py-16 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-900 transition-colors">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center max-w-3xl mx-auto mb-16 space-y-4">
                ...
                <p class="text-slate-500 text-sm sm:text-base">Jelajahi setiap fasilitas menarik yang siap melengkapi momen rekreasi dan pembelajaran Anda.</p>
            </div>
            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    @foreach($facilities as $facility)
                        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl overflow-hidden hover:shadow-lg transition-all duration-300 flex flex-col justify-between">
                            <div>
                                <div class="w-full h-52 bg-slate-100 dark:bg-slate-950 overflow-hidden relative">
                                    <img src="{{ $facility->image_path ?? 'https://images.unsplash.com/photo-1500937386664-56d159f8e281?auto=format&fit=crop&w=500&q=80' }}" alt="{{ $facility->name }}" class="w-full h-full object-cover" />
                                </div>
                                <div class="p-6 space-y-3">
                                    <h3 class="font-bold text-slate-900 dark:text-white text-lg">{{ $facility->name }}</h3>
                                    <p class="text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{{ $facility->description }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
